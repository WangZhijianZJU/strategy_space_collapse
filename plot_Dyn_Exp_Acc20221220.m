% >> ofilehead = 'Exp'; plot_Dyn_Exp_Acc20221220(ofilehead);
% >> ofilehead = 'Dyn'; plot_Dyn_Exp_Acc20221220(ofilehead);
% C:\Users\think\Documents\MATLAB\plot_Dyn_Exp_Acc20221220.m
function r = plot_Dyn_Exp_Acc20221220(ofilehead)
%     close all 
%     clear 
                                      p=[6 8 10];             leg ={'(2,1)', '(3,2)', '(4,2)'}
 %% ----------------------------------------------------------------------------------------------------------------%%   
 if ofilehead == 'Exp'     
...
%          subplot(1,2,1); 
         plot(Acc_3page(:,1:8,k),'linewidth',4); xlim([0,400]);ylim([0 1500]);axis square; text(20,980,strcat(ofilehead,'-',leg(k),'-X'),'fontsize',16)
%            legend('X_1', 'X_2', 'X_3', 'X_4', 'X_5', 'X_6', 'X_7', 'X_8','location','eastoutside') 
         ...
         figure; 
%          subplot(1,2,2); 
         plot(Acc_3page(:,9:16,k),'linewidth',4);xlim([0,400]);ylim([0 1500]);axis square; text(20,980,strcat(ofilehead,'-',leg(k),'-Y'),'fontsize',16)
